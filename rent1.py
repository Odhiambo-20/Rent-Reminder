...
def add_tenant(name, email, rent_due_date, rent_amount):
    cursor.execute('''
    INSERT INTO tenants (name, email, rent_due_date, rent_amount)
    VALUES (?, ?, ?, ?)
    ''', (name, email, rent_due_date, rent_amount))
    conn.commit()
    print(f"Tenant {name} added successfully.")

# Function to send email notifications
def send_email(to_address, subject, message):
    from_address = "odhiamboondiko@gmail.com"
    password = "Victor@2001"
...
# Example usage
def main():
    # Add a new tenant (Example: John Doe)
    add_tenant("Victor Odhiambo", "odhiamboondiko@gmail.com", "2024-09-01", 1200.00)
